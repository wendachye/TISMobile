...
import static com.bizconnectivity.tismobile.webservices.ConstantWS.WS_TS_GET_JOB_DETAIL_BY_ORDER_ID;

public class OrderIDSearchWS {

	public static ArrayList<JobDetail> invokeRetrieveJobDetailByOrderID (String timeslotID) {

		ArrayList<JobDetail> jobDetailArrayList = new ArrayList<>();

		//create request
		SoapObject request = new SoapObject(NAMESPACE, WS_TS_GET_JOB_DETAIL_BY_ORDER_ID);

		//property which holds input parameters
		PropertyInfo propertyInfoTI = new PropertyInfo();
		//set name
		propertyInfoTI.setName("timeslotID");
		//set value
		propertyInfoTI.setValue(timeslotID);
		//set datatype
		...
		try {
			//invoke web service
			androidHTTPTransport.call(SOAP_ACTION + WS_TS_GET_JOB_DETAIL_BY_ORDER_ID, envelope);
